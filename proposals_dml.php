...
// Data functions (insert, update, delete, form) for table proposals

// This script and data application were generated by AppGini 5.72
// Download AppGini for free from https://bigprof.com/appgini/download/

function proposals_insert(){
	global $Translation;

	// mm: can member insert record?
	$arrPerm=getTablePermissions('proposals');
	if(!$arrPerm[1]){
		return false;
	}
	...
	// hook: proposals_before_insert
	if(function_exists('proposals_before_insert')){
		$args=array();
		if(!proposals_before_insert($data, getMemberInfo(), $args)){ return false; }
	}

	$o = array('silentErrors' => true);
	sql('insert into `proposals` set       `notice`=' . (($data['notice'] !== '' && $data['notice'] !== NULL) ? "'{$data['notice']}'" : 'NULL') . ', `manager`=' . (($data['manager'] !== '' && $data['manager'] !== NULL) ? "'{$data['manager']}'" : 'NULL') . ', `project`=' . (($data['project'] !== '' && $data['project'] !== NULL) ? "'{$data['project']}'" : 'NULL') . ', `proposal`=' . (($data['proposal'] !== '' && $data['proposal'] !== NULL) ? "'{$data['proposal']}'" : 'NULL') . ', `date`=' . "'{$data['date']}'", $o);
	if($o['error']!=''){
		echo $o['error'];
		echo "<a href=\"proposals_view.php?addNew_x=1\">{$Translation['< back']}</a>";
		exit;
	}

	$recID = db_insert_id(db_link());

	// hook: proposals_after_insert
	if(function_exists('proposals_after_insert')){
		$res = sql("select * from `proposals` where `id`='" . makeSafe($recID, false) . "' limit 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = makeSafe($recID, false);
		$args=array();
		if(!proposals_after_insert($data, getMemberInfo(), $args)){ return $recID; }
	}

	// mm: save ownership data
	set_record_owner('proposals', $recID, getLoggedMemberID());

	return $recID;
}

function proposals_delete($selected_id, $AllowDeleteOfParents=false, $skipChecks=false){
	...
	$arrPerm=getTablePermissions('proposals');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='proposals' and pkValue='$selected_id'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='proposals' and pkValue='$selected_id'");
	...
	// hook: proposals_before_delete
	if(function_exists('proposals_before_delete')){
		$args=array();
		if(!proposals_before_delete($selected_id, $skipChecks, getMemberInfo(), $args))
			return $Translation['Couldn\'t delete this record'];
	}

	sql("delete from `proposals` where `id`='$selected_id'", $eo);

	// hook: proposals_after_delete
	if(function_exists('proposals_after_delete')){
		$args=array();
		proposals_after_delete($selected_id, getMemberInfo(), $args);
	}

	// mm: delete ownership data
	sql("delete from membership_userrecords where tableName='proposals' and pkValue='$selected_id'", $eo);
}

function proposals_update($selected_id){
	global $Translation;

	// mm: can member edit record?
	$arrPerm=getTablePermissions('proposals');
	$ownerGroupID=sqlValue("select groupID from membership_userrecords where tableName='proposals' and pkValue='".makeSafe($selected_id)."'");
	$ownerMemberID=sqlValue("select lcase(memberID) from membership_userrecords where tableName='proposals' and pkValue='".makeSafe($selected_id)."'");
	...
	// hook: proposals_before_update
	if(function_exists('proposals_before_update')){
		$args=array();
		if(!proposals_before_update($data, getMemberInfo(), $args)){ return false; }
	}

	$o=array('silentErrors' => true);
	sql('update `proposals` set       `notice`=' . (($data['notice'] !== '' && $data['notice'] !== NULL) ? "'{$data['notice']}'" : 'NULL') . ', `manager`=' . (($data['manager'] !== '' && $data['manager'] !== NULL) ? "'{$data['manager']}'" : 'NULL') . ', `project`=' . (($data['project'] !== '' && $data['project'] !== NULL) ? "'{$data['project']}'" : 'NULL') . ', `proposal`=' . (($data['proposal'] !== '' && $data['proposal'] !== NULL) ? "'{$data['proposal']}'" : 'NULL') . ', `date`=`date`' . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
		echo '<a href="proposals_view.php?SelectedID='.urlencode($selected_id)."\">{$Translation['< back']}</a>";
		exit;
	}


	// hook: proposals_after_update
	if(function_exists('proposals_after_update')){
		$res = sql("SELECT * FROM `proposals` WHERE `id`='{$data['selectedID']}' LIMIT 1", $eo);
		if($row = db_fetch_assoc($res)){
			$data = array_map('makeSafe', $row);
		}
		$data['selectedID'] = $data['id'];
		$args = array();
		if(!proposals_after_update($data, getMemberInfo(), $args)){ return; }
	}

	// mm: update ownership data
	sql("update membership_userrecords set dateUpdated='".time()."' where tableName='proposals' and pkValue='".makeSafe($selected_id)."'", $eo);

}

function proposals_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if(is_file(dirname(__FILE__).'/hooks/proposals.proposal.csv')){
		$proposal_data = addslashes(implode('', @file(dirname(__FILE__).'/hooks/proposals.proposal.csv')));
	...
	if($selected_id){
		...
		$res = sql("select * from `proposals` where `id`='".makeSafe($selected_id)."'", $eo);
		if(!($row = db_fetch_array($res))){
			return error_message($Translation['No records found'], 'proposals_view.php', false);
		}
			...
			$j("#manager-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_manager__RAND__.value, t: 'proposals', f: 'manager' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'proposals', f: 'manager' }; },
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#project-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_manager: filterer_manager, id: AppGini.current_project__RAND__.value, t: 'proposals', f: 'project' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_manager: filterer_manager, s: term, p: page, t: 'proposals', f: 'project' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#project-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_project__RAND__.value, t: 'proposals', f: 'project' },
					success: function(resp){
						$j('[name="project"]').val(resp.results[0].id);
						$j('[id=project-container-readonly__RAND__]').html('<span id="project-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=categories_view_parent]').hide(); }else{ $j('.btn[id=categories_view_parent]').show(); }
	...
	if($dvprint){
		$template_file = is_file("./{$TemplateDVP}") ? "./{$TemplateDVP}" : './templates/proposals_templateDVP.html';
		$templateCode = @file_get_contents($template_file);
	}else{
		$template_file = is_file("./{$TemplateDV}") ? "./{$TemplateDV}" : './templates/proposals_templateDV.html';
		$templateCode = @file_get_contents($template_file);
	}
	...
	// process buttons
	if($AllowInsert){
		if(!$selected_id) $templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-success" id="insert" name="insert_x" value="1" onclick="return proposals_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save New'] . '</button>', $templateCode);
		$templateCode = str_replace('<%%INSERT_BUTTON%%>', '<button type="submit" class="btn btn-default" id="insert" name="insert_x" value="1" onclick="return proposals_validateData();"><i class="glyphicon glyphicon-plus-sign"></i> ' . $Translation['Save As Copy'] . '</button>', $templateCode);
	}else{
	...
	if($selected_id){
		...
		if($AllowUpdate){
			$templateCode = str_replace('<%%UPDATE_BUTTON%%>', '<button type="submit" class="btn btn-success btn-lg" id="update" name="update_x" value="1" onclick="return proposals_validateData();" title="' . html_attr($Translation['Save Changes']) . '"><i class="glyphicon glyphicon-ok"></i> ' . $Translation['Save Changes'] . '</button>', $templateCode);
		}else{
	...
	$rdata = $jdata = get_defaults('proposals');
	if($selected_id){
		$jdata = get_joined_record('proposals', $selected_id);
		if($jdata === false) $jdata = get_defaults('proposals');
		$rdata = $row;
	}
	$templateCode .= loadView('proposals-ajax-cache', array('rdata' => $rdata, 'jdata' => $jdata));

	// hook: proposals_dv
	if(function_exists('proposals_dv')){
		$args=array();
		proposals_dv(($selected_id ? $selected_id : FALSE), getMemberInfo(), $templateCode, $args);
	}

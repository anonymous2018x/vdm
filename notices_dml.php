...
function notices_insert(){
	...
	sql('insert into `notices` set       `notice`=' . (($data['notice'] !== '' && $data['notice'] !== NULL) ? "'{$data['notice']}'" : 'NULL') . ', `manager`=' . (($data['manager'] !== '' && $data['manager'] !== NULL) ? "'{$data['manager']}'" : 'NULL') . ', `project`=' . (($data['project'] !== '' && $data['project'] !== NULL) ? "'{$data['project']}'" : 'NULL') . ', `worker`=' . (($data['worker'] !== '' && $data['worker'] !== NULL) ? "'{$data['worker']}'" : 'NULL') . ', `date`=' . "'{$data['date']}'", $o);
	if($o['error']!=''){
		echo $o['error'];
...
function notices_update($selected_id){
	...
	$o=array('silentErrors' => true);
	sql('update `notices` set       `notice`=' . (($data['notice'] !== '' && $data['notice'] !== NULL) ? "'{$data['notice']}'" : 'NULL') . ', `manager`=' . (($data['manager'] !== '' && $data['manager'] !== NULL) ? "'{$data['manager']}'" : 'NULL') . ', `project`=' . (($data['project'] !== '' && $data['project'] !== NULL) ? "'{$data['project']}'" : 'NULL') . ', `worker`=' . (($data['worker'] !== '' && $data['worker'] !== NULL) ? "'{$data['worker']}'" : 'NULL') . ', `date`=`date`' . " where `id`='".makeSafe($selected_id)."'", $o);
	if($o['error']!=''){
		echo $o['error'];
...
function notices_form($selected_id = '', $AllowUpdate = 1, $AllowInsert = 1, $AllowDelete = 1, $ShowCancel = 0, $TemplateDV = '', $TemplateDVP = ''){
	...
	if(is_file(dirname(__FILE__).'/hooks/notices.worker.csv')){
		$worker_data = addslashes(implode('', @file(dirname(__FILE__).'/hooks/notices.worker.csv')));
			...
			$j("#manager-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { id: AppGini.current_manager__RAND__.value, t: 'notices', f: 'manager' },
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { s: term, p: page, t: 'notices', f: 'manager' }; },
		...
		<?php if(($AllowUpdate || $AllowInsert) && !$dvprint){ ?>
			...
			$j("#project-container__RAND__").select2({
				...
				initSelection: function(e, c){
					$j.ajax({
						url: 'ajax_combo.php',
						dataType: 'json',
						data: { filterer_manager: filterer_manager, id: AppGini.current_project__RAND__.value, t: 'notices', f: 'project' },
						success: function(resp){
							c({
								id: resp.results[0].id,
								text: resp.results[0].text
				...
				ajax: {
					...
					dataType: 'json',
					cache: true,
					data: function(term, page){ /* */ return { filterer_manager: filterer_manager, s: term, p: page, t: 'notices', f: 'project' }; },
					results: function(resp, page){ /* */ return resp; }
				},
				escapeMarkup: function(str){ /* */ return str; }
			}).on('change', function(e){
			...
			if(!$j("#project-container__RAND__").length){
				$j.ajax({
					url: 'ajax_combo.php',
					dataType: 'json',
					data: { id: AppGini.current_project__RAND__.value, t: 'notices', f: 'project' },
					success: function(resp){
						$j('[name="project"]').val(resp.results[0].id);
						$j('[id=project-container-readonly__RAND__]').html('<span id="project-match-text">' + resp.results[0].text + '</span>');
						if(resp.results[0].id == '<?php echo empty_lookup_value; ?>'){ $j('.btn[id=projects_view_parent]').hide(); }else{ $j('.btn[id=projects_view_parent]').show(); }
